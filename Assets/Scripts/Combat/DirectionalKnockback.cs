...
public class DirectionalKnockback : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        // 处理跳跃状态
        CharacterJump characterJump = controller.gameObject.GetComponent<Character>()?.FindAbility<CharacterJump>();
        ...
        if (ShowDebugInfo)
        {
            Debug.Log($"[DirectionalKnockback] 弹幕速度: {_velocity}, 击退方向: {knockbackDirection}, 最终击退力: {finalKnockbackForce}");
        }
    }
